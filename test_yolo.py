...
def test(args: dict):
    ...
    try:
        ...
        # YOLO handles gradients internally during inference, 
        # but wrapping in torch.no_grad() is still good practice
        with torch.no_grad():
            ...
            for x_rgb, labels in pbar_val:
                ...
                # verbose=False prevents YOLO from printing logs for every single batch
                outputs = model(x_rgb, verbose=False) 
                
                # Extract predictions from the YOLO 'Results' object list
                # r.probs.top1 gets the predicted class index for each image in the batch
        ...
        for color in colors:
            for model_name in models:
                ...
                args = {
                    ...
                    "weights_path": f'runs/classify/{model_name}_{color}/cls/weights/best.pt',
                    "model": model_name,
                    "image_mode": color,
